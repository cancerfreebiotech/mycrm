...
- **AI（OCR / 助理 / 潤稿等）**：Google Gemini 系列，經 `src/lib/aiRouting.ts` 依「AI 功能 × 端點/金鑰/模型」組織指派動態解析（未指派走系統預設；Portkey 為主要 gateway，另支援 OpenAI 相容端點）
- **圖片處理**：Sharp
- **國際化**：next-intl
- **主題切換**：next-themes
- **套件管理**：npm
...
- AI 路由層（功能→端點/模型解析）: `src/lib/aiRouting.ts`
- OpenAI 相容工具迴圈: `src/lib/openaiAgent.ts`
- Middleware（注意檔名）: `src/proxy.ts`（不是 `middleware.ts`）
...
| PATCH 進位到兩位數（會 ≥ 10） | MINOR +1，PATCH 歸零 | 6.4.9 → 6.5.0（不是 6.4.10） |
| MINOR 進位到兩位數（會 ≥ 10） | MAJOR +1，MINOR/PATCH 歸零 | 1.9.x → 2.0.0 |

**核心原則**：PATCH 跟 MINOR 都不允許出現兩位數，要進位時直接帶動上一級歸零。
