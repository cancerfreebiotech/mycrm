...
**僅限 super_admin**：AI 模型、系統健康、MCP 活動、MCP 權杖、Email 復活、共用 Email、意見回饋、退訂管理、資料查詢（DSAR）、電子報總覽。
...
| [稽核日誌](audit-log.md) | 檢視角色變更、刪除等特權操作的稽核紀錄；可依操作者／動作／日期篩選並匯出 CSV |
| [組織設定與品牌](org-settings.md) | 全公司名稱、登入網域、電子報品牌、回報通知收件人，以及模組開關（可一鍵停用 Hunter 補全、AI 助理） |
| 退訂管理 | 輸入任一 email 立即查出「能不能寄、為什麼」，彙整五個抑制來源（退訂、黑名單、退信等） |
| 資料查詢（DSAR） | 以 email 聚合所有相關聯絡人（含已刪除）與個資足跡，可一鍵匯出，查詢自動留稽核 |
| 電子報總覽 | 近 12 次寄送的開信／點擊趨勢、各名單健康度（含 180 天未開信）、訂閱／退訂／黑名單總數 |
