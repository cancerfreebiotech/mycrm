...
## v1.2 — UX 強化 + 國家欄位 + 寄信強化（待實作）

### 變更項目
- **Email copy 按鈕**：聯絡人列表和詳情頁 email 旁加 📋 按鈕，點擊複製並顯示「✅ 已複製」提示
- **多張名片上傳（最多 6 張）**：新增聯絡人和編輯名片管理均支援，壓縮後合併送 Gemini 辨識，AI 自動選最完整欄位，新增左右對照確認介面
- **編輯聯絡人頁面重構**：文字欄位與名片管理分離，名片管理獨立區塊
- **新增國家欄位**：`contacts.country_code`，AI 依電話國碼 / 地址 / 語言特徵自動辨識
- **`countries` 資料表**：super_admin 管理，預設台灣、日本、美國、韓國、新加坡、印度，支援多語言名稱
- **新增 `/admin/countries` 頁面**（僅 super_admin）
- **全新寄信介面**：大型 Modal，支援 AI 寫信（帶入收件人資訊 + 最近 1 筆互動紀錄）、template 選擇（附件自動帶入）、臨時附件上傳（每檔 2MB）
- **手機 sidebar**：< 768px 顯示 hamburger 滑出式選單；768–1024px 收縮為 icon-only，可展開
- **全站圖片壓縮統一規範**：所有上傳點一律 1024px / JPEG Q85
- **`/docs` 同步更新**：反映 v1.2 所有新功能
