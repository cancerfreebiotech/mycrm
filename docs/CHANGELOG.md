...
### 安全／授權
- **名片王(camcard) API 補上伺服器端功能授權**：`/api/camcard/*`（待審清單、指派、確認、合併、更新、略過）過去只靠登入+MFA 放行、未檢查 `camcard` 功能授權，未獲授權的員工可越權操作。新增通用 `hasFeatureAccess()`（`src/lib/featureAccess.ts`），全部端點補上 403 閘門。
- **電子報活動明細 GET 補授權閘門**：`GET /api/newsletter/campaigns/[id]` 先前只驗登入（改寫/刪除等已在上一版補），現一併要求 `newsletter` 授權。

### 正確性／資料完整性
- **名片王確認／合併防重複**：`confirm`／`merge` 加原子狀態認領（`pending→confirmed` 條件式更新），重複提交（雙擊／重試）不再建立重複聯絡人；後續失敗會還原為 `pending` 保持可重試。
- **電子報寄送併發防重複**：寄送 worker 每個 chunk 送出前即時重查已寄清單並過濾，重疊／重試的寄送不再對同一收件人重複寄信。
- **聯絡人合照顯示修復**：私有 bucket 的照片先前用未簽名 URL 顯示導致破圖，改為與名片一樣簽名後顯示（含燈箱與寄信 Modal 縮圖）。
- **舊版名片旋轉按鈕**：舊資料無真實 card id、旋轉必定 404，改為不顯示該動作。
- **搜尋注入防護**：聯絡人合併搜尋、名片王待審搜尋含逗號／括號等特殊字元不再破壞查詢（共用 `orQuote()`）。
- **合併搜尋競態**：慢回應不再覆蓋較新的搜尋結果。
- **寄信互動紀錄**：多收件人寄信後回填時間軸改以 `contact_id` 精確比對，不會把他人的 email 紀錄誤置到本聯絡人頁。

### i18n
- 聯絡人列表的重要性／email 狀態／語言等篩選與批次表單的硬編標籤（ALL、English 等）改走 i18n（三語同步新增 `common.all`）。

### 其他
- Telegram bot 相簿計數訊息在多圖並發時可能卡在「1」，改為有界重讀後更新為實際張數（照片本身一向完整保存）。
...
### 變更項目
- **修正競態**：AI 助理聊天在歷史載回前送出訊息會被覆蓋、會議前 Briefing 掛載載回與手動產生並發衝突——兩處掛載水合皆改為「使用者已互動就不覆蓋」。
- **修正**：AI 助理走 OpenAI 相容端點時空回覆補上兜底訊息（與 Google 路徑一致）；Telegram bot 的工具迴圈達上限訊息改用使用者語言（原僅繁中）。
- **管理頁誠實顯示**：功能指派的模型（或其端點）被停用時，該列顯示黃色提示「實際改走系統預設」，不再看起來像未指派。
- **手冊**：super_admin 三語補充「端點測試使用最新建立的啟用模型、失敗訊息以模型 ID 開頭時代表模型問題」；並將 `docs_content` 全部 9 列自 repo 來源完整重新同步（md5 逐列對帳）——修復 07-11 凌晨被外部自動化改寫成精簡版的問題。
...
### 變更項目
- **聯絡人頁的會議前 Briefing 現在會自動載回已存結果**：Briefing 一直有存在資料庫（`contact_briefings`），但頁面重整後只顯示「產生」按鈕、看起來像每次都要重新生成。現在掛載時自動顯示該聯絡人最新一份（含產生時間與使用模型），生成中的也會接續輪詢；按「重新產生」才會再跑一次。
- 新增 `GET /api/social-briefing/latest?contactId=`（查該聯絡人最新一份 briefing）。
- 使用手冊三語同步更新。

## v8.1.2 — AI 助理對話持久化（2026-07-09）

### 變更項目
- **AI 助理（網頁版）對話自動保存**：重新整理、關閉頁面或換裝置後對話仍在（保留最近 40 則，新表 `ai_chat_sessions`，僅 API 以 service role 存取）。
- 對話區新增「清除對話」按鈕（行內二次確認）。
- `/api/ai-chat` 新增 GET（載回歷史）與 DELETE（清除）；POST 成功後自動保存，保存失敗不影響回覆。
- 使用手冊三語同步更新（AI 助理限制段落）。

## v8.1.1 — 修正中文輸入法 Enter 選字誤送出（2026-07-09）

### 變更項目
- **修正**：Mac 注音／日文等輸入法組字中按 Enter「選字確認」時，AI 助理聊天框會把整句提前送出的問題。新增共用防護 `src/lib/imeGuard.ts`（涵蓋 Chrome `isComposing`／`keyCode 229` 與 Safari WebKit bug 165004 的 compositionend 時序補判），全站 **21 處** Enter 送出／儲存輸入框一併修正（聊天框、標籤、國家、範本、電子報名單／草稿／活動、聯絡人頁各輸入框、MFA 驗證、設定頁、稽核日誌、TipTap 連結輸入等）。
...
### 變更項目
- `/admin/models` 端點列的**服務商名稱**可點鉛筆行內編輯（Enter 或「儲存」送出；功能指派下拉的「端點／模型」標籤同步更新）。
- 手冊三語同步更新該段說明（Base URL 維持建立後不可改）。
...
### 變更項目
- **電子報行內編輯器**（回應使用者回饋「無法更改字體、貼上的文字與現有字體無法統一」）：
  - 貼上 Word／網頁等外部文字時，自動去除自帶的字體、字級、顏色等樣式，貼上內容直接與信件內文一致。
  - 工具列新增「字體」選單（預設字體＝與信件範本一致的字體堆疊，另有 Arial／Georgia／Times New Roman／Verdana）。
  - 工具列新增「清除格式」按鈕，可將先前貼壞的段落一鍵洗回信件預設樣式。
- **意見回饋確認制**：所有回報改由回報者本人結案。
  - 管理端新增狀態「已處理，待確認」，且不能再直接設「已完成」；設定時系統自動依回報者的介面語言寄 Email 通知。
  - `/feedback` 頁新增「我的回饋」清單：追蹤自己每筆回報的狀態，已處理項目由本人按「確認完成」結案。
  - 資料庫：`feedback.status` 新增 `resolved`；新增 RLS policy `feedback_confirm_own`（回報者僅能把自己的 resolved 改為 done）。
- 使用手冊（/docs）三語同步更新（電子報行內編輯、意見回饋、回饋管理章節）。
...
### 變更項目
- **A/B 測試模式可選**（quick-send 編輯頁）：「小樣本＋自動送贏家」（預設）或「全名單 50/50 對半」——舊 50/50 模式重新有了 UI 入口，既有的 legacy 活動載入時自動歸類。
- **Hunter 停用提示**：模組於組織設定關閉時，系統健康頁的批次補全顯示明確的黃色「模組已停用」提示與設定連結，不再顯示易誤判的一般零結果。
- **活動分析計數對齊**：分析頁的 總數/成功/失敗 改為與寄送引擎相同的「去重收件人」語意——重試過的活動不再灌水、開信率分母正確。
- **電子報總覽提速**：名單健康度改由資料庫端聚合（單一 RPC），大名單不再有逾時風險；數字語意不變。
- **/api/me 提速**：省去重複的身分解析查詢（hot endpoint 少兩次 DB 往返）。
- **批次上傳互動紀錄 i18n 化**（原寫死中文）。
- **使用手冊三語同步**（21 檔）：組織設定 7 新欄、預算門檻、停權即時生效、稽核篩選與台北時區、A/B 測試模式、`/v` 多字姓名、僅會議日摘要、Hunter 停用提示。
- 新增 `UAT-v7.9.md` 驗收清單（v7.8.1 → v7.9.6 需真人實測項目）。
...
### 安全與隱私
- **DSAR / 寄送資格查詢的 ILIKE 萬用字元洩漏**：email 中常見的 `_`/`%` 先前被當作 SQL 萬用字元，可能誤配到**別人**的聯絡人／黑名單資料（如 `mary_jane@` 配到 `maryzjane@`）。已統一跳脫；`escapeLikePattern` 抽為共用 `src/lib/likeEscape.ts`，收斂原本三處重複定義。
- **模組開關補齊路徑**：「AI 助理」開關先前只擋網頁版，Telegram `/ai` 照常運作；「Hunter 補全」開關只擋 inbound-parse，排程與管理批次照常燒 API 額度。現在所有路徑都遵守開關。
- **使用者停權靜默失效**：目標使用者沒有 organization_members 列時，停權 UPDATE 影響 0 列仍回報成功、被停權者照常登入。現在會自動補建該列，停權即時生效。

### Telegram Bot
- **`/v` 多字姓名解析回歸**：先前在第一個空白斷詞，`/v John Smith` 會把「Smith」當筆記存進（可能錯誤的）聯絡人。現在整串優先、再由長到短嘗試前 4 個斷點，第一個唯一命中者勝出；長備註也不影響姓名解析。

### 電子報
- **A/B 贏家發送中斷不再遺留名單**：先前 winner 蓋章在最終發送**之前**,發送逾時後該活動不再被 cron 撿起，剩餘訂閱者永遠收不到信。現在最終發送可跨 cron 續跑至完成。
- **活動計數冪等**：final 重跑先前會重複累加 `total_recipients`，稀釋開信率分母。現在由 recipients 實際資料重新推導；讀取失敗時保留既有計數（不寫 0）。
- **UTM 連結實體編碼**：自動加掛的 utm 參數現在一律以 `&amp;` 寫入 href，不再產生無效 HTML。
- **總覽儀表板大名單計數**：分頁掃描補上穩定排序（>1000 列不再重複/漏算）並設定 `maxDuration`。

### 任務提醒
- **「今日會議」不再被任務清單綁架**：先前沒有到期任務就整封摘要跳過，只有會議的日子收不到會議摘要。現在有任務**或**有會議即寄送。

### 稽核日誌
- **日期篩選改錨定台北時區**：先前以 UTC 為日界，篩選/CSV 匯出會漏掉當日 00:00–07:59 的操作。
- **補上新動作的標籤**：`dsar_lookup`、`set_status`、`newsletter_send` 加入篩選下拉與三語標籤，不再顯示原始 key。

## v7.8.0 — feat(crm/newsletter/ai/ops): 稽核 backlog 22 項功能全數實裝（2026-07-04）

### 業務效率
- **`/v` 一句話記錄拜訪**：Telegram Bot 現在支援 `/v 王醫師 昨天聊了合作`——AI 自動解析日期/地點/內容並寫入互動紀錄，多位同名會出按鈕選擇；原本的逐步流程保留。
- **任務提醒可直接按按鈕處理**：每日 09:00 的任務摘要，每項任務附「✅ 完成」「⏰ +1天」按鈕，不用再開網頁。
- **摘要加入「🗓 今日會議」**：自動列出今天的 Outlook 會議、連結對應聯絡人，briefing 準備好會標示。
- **冷關係提醒**：每週一早上自動提醒你「該聯繫了」——重要聯絡人 30 天、一般 90 天未互動就會出現（已有進行中任務的不重複提醒）。
- **會前 briefing 自動送到 Telegram**：briefing 完成後直接推播摘要+連結；會議結束後還會提示你用 `/v` 一句話記錄成果。
- **AI 助理新工具**：問 AI「我和某人目前的關係如何」可直接得到互動脈絡總結（網頁與 Telegram 皆可）。
- **重複聯絡人 AI 合併建議**：重複管理頁新增「AI 建議」——自動判斷該保留哪筆、標出會被捨棄的欄位值，一鍵依建議合併。

### 電子報
- **A/B 主旨測試升級為「小樣本+自動送贏家」**：先寄給測試組（預設 20%，A/B 各半），等待期後系統自動用開信率較高的主旨寄給其餘所有人。
- **自動 UTM 標記**：電子報內所有外部連結自動加上 utm 參數，網站流量可歸因到活動。
- **寄送失敗明細**：活動分析頁顯示每位失敗收件人的具體錯誤原因。
- **逐名單退訂**：收件人點退訂連結後可只取消特定名單，不必全部退光；舊連結行為不變。
- **電子報總覽儀表板**：近 12 次寄送的開信/點擊趨勢、各名單健康度（180 天未開信人數）、訂閱/退訂/黑名單總數。

### 管理與合規
- **退訂與寄送資格總表**（`/admin/suppressions`）：輸入任一 email 立即知道「能不能寄、為什麼」，五個抑制來源一次看。
- **資料主體查詢 DSAR**（`/admin/dsar`）：以 email 聚合所有相關聯絡人（含已刪除）與個資足跡，一鍵匯出，查詢自動留稽核。
- **使用者停用/離職**：管理頁可停用帳號，停用者下次登入即被擋下；Super Admin 受保護不可停用。
- **稽核日誌強化**：可依操作者/動作/日期篩選並匯出 CSV；Webhook 設定、發版通知、聯絡人合併等操作補上稽核。
- **組織設定更完整**：回報通知收件人可設定；新增「模組開關」（Hunter 補全、AI 助理可一鍵停用）。

### 維運
- **Bot 錯誤不再靜默**：Telegram Bot 例外寫入 dead-letter 並顯示在系統健康頁。
- **用量預算告警**：可為 AI 呼叫/Email/電子報設定每月上限，達 80%/100% 自動通知管理員。
- **cron_runs 自動清理**：心跳紀錄保留 30 天，不再無限累積。
...
### 安全性 / 權限
- **角色與功能授權改為後端稽核**：後台「使用者管理」的升 super_admin／授權功能，先前由瀏覽器直接寫 DB，現改走 `/api/admin/users/[id]/access`（強制 super_admin、寫入 `admin_actions` 稽核日誌），並在後端擋下對 Super Admin 帳號的降級。（DB 層 trigger 保護待管理員核准後補上。）

### 組織設定實際生效
- **電子報 AI 潤稿／宣傳語改讀「組織名稱」設定**：先前 prompt 寫死公司名，現在 `/admin/org-settings` 的組織名稱會真正反映在 AI 產出的電子報文字。
- **電子報寄送套用「回覆地址」設定**：寄出的電子報現在帶 `reply_to`（取自組織設定），收件人回信會寄到設定的信箱，而不是只有寄件地址。

### 文件
- 全站說明文件對齊到 v7.7.0（三語同步）：修正任務到期提醒說明（每日 09:00 Telegram 摘要，非每分鐘掃描）、補上電子報排程寄送／A/B 主旨／互動分群／成效分析、會前自動 briefing、聯絡人列表批次操作、個人 Email prompt、組織設定／稽核日誌／GDPR 匯出；補齊部署文件遺漏的 5 個 cron 與多個環境變數；Bot 指令表補 `/meet`、`/met` 並修正 `/help` 內錯誤指令名。

## v7.7.0 — feat(saas/compliance/docs): 組織設定頁 + GDPR 匯出與刪除防復活 + 多租戶鷹架 + 管理文件補齊（2026-07-04）

### 組織設定與品牌（新管理頁）
- 新增 `/admin/org-settings`：公司名稱、允許登入網域、電子報 Logo／回信位址／社群連結集中管理——改品牌不再需要改程式重新部署。電子報產出與登入網域驗證已接上（未設定時行為與現在完全相同）。

### GDPR / 個資合規
- **聯絡人資料匯出**：新增單一聯絡人全量資料匯出 API（主檔、名片、相片、互動、任務、寄送紀錄），限有匯出權限者，操作留稽核。
- **刪除防復活**：永久刪除的聯絡人現在會留下抹除標記——先前隔天就可能被 Email 自動擷取或 Hunter 補全用同一信箱重新建立，形同刪除無效。現已在所有自動建立路徑擋下。

### v8.0 多租戶鷹架（Phase 0，零行為改變）
- 建立 organizations／organization_members 資料表（全員已入籍預設組織）、`getOrgContext()` 基礎設施，以及 org-ready 技術債報表（`npm run lint:org`，現況 59/117 支 API 待遷移）——為 SaaS 化鋪路，今日行為完全不變。

### 文件
- **15 個管理頁面全部有了專屬說明文件**（三語）：名片批次審核、重複聯絡人、系統健康、MCP 權杖、稽核日誌、組織設定等,說明書目錄同步更新。

## v7.6.0 — feat(crm/ai): 任務提醒 + 聯絡人頁任務面板 + Bot AI 助理 + 會前自動 Briefing 等 9 項（2026-07-04）

### 不再漏掉跟進
- **Telegram 每日任務摘要**（09:00）：有逾期或今日到期任務的人會收到清單推播；沒有任務就不打擾。
- **掃名片建檔後一鍵跟進**：Bot 確認存檔訊息上新增「3 天後跟進」「1 週後跟進」按鈕，點一下自動建立關聯任務。
- **聯絡人頁「相關任務」面板**：直接在聯絡人頁查看、新增、勾選完成該聯絡人的跟進任務。

### 聯絡人清單強化
- **「無互動時間」篩選**：一鍵找出 30／60／90 天以上沒有互動的聯絡人（從未互動也會命中）。
- **儲存檢視**：把目前的整組篩選存成具名檢視，之後一鍵套用。

### AI 加值
- **Bot 真 AI 助理**：Telegram 輸入 `/ai 問題` 即可用自然語言查詢/操作 CRM（與網頁版 AI 助理同一套工具，含稽核）。
- **AI 依聯絡人脈絡起草 email**：聯絡人頁的 AI 草稿現在會帶入該聯絡人的公司/職稱/標籤/近期互動/最新 Briefing。
- **會前自動 Briefing**：每 6 小時掃描 Outlook 行事曆，未來 24 小時會議的外部與會者自動排入 AI 簡報，完成後在聯絡人頁可看，並發 Telegram 通知。
- **重複聯絡人 AI 判斷**：重複管理頁一鍵請 AI 判定是否同一人（含理由與信心），可直接採用建議合併（保留既有二次確認）。
- **未指派筆記 AI 配對**：AI 讀筆記內容建議該掛到哪位聯絡人，點選即完成指派。

## v7.5.0 — feat(newsletter): 排程寄送 + 活動分析頁 + 互動分群 + A/B 主旨測試（2026-07-04）

### 排程寄送
- 快速寄送頁新增「**排程寄送**」：選定日期時間後系統自動寄出（每 10 分鐘檢查），可隨時取消排程。寄送核心抽取為共用模組，排程與手動寄送走同一套（含斷點續寄與重複防護）。

### 活動分析頁
- 每個已寄出的活動新增**分析頁**：寄出／失敗／開信率／點擊率統計卡、**連結層級點擊表**（每個連結的點擊次數與獨立人數）、開信/點擊**時間軸**、退信/退訂/垃圾回報事件數。事件明細自 v7.2.9 起收集，較舊活動會顯示說明。

### 互動分群
- 活動列表可從已寄出活動一鍵建立分群名單：**開信者／點擊者／未開信**——「補寄給沒開信的人」現在是兩個點擊的事。

### A/B 主旨測試
- 撰寫頁新增「主旨 B」欄位：填寫後寄送時自動 50/50 分流兩種主旨，分析頁顯示各版本開信率。

## v7.4.0 — feat(ops): 維運儀表板 + 修復所有排程任務被攔截的重大問題（2026-07-04）

### 重大修復
- **所有 Vercel 排程任務（cron）從未真正執行**：登入攔截層未放行 `/api/cron/*` 等自帶驗證的路徑，排程請求一律被轉向登入頁（runtime logs 實證每 2 分鐘 307）。名片辨識與 AI 簡報一直靠即時處理路徑支撐，排程後援、每日意見回饋摘要、Hunter 排程等實際上全數失效。現已放行（各路徑仍以 CRON_SECRET／API 權杖自我驗證）。

### 維運儀表板（系統健康頁新增三區塊）
- **排程心跳**：每個排程任務的最後執行時間、狀態與耗時；逾時／異常一眼可見。
- **死信佇列**:各處理佇列的失敗數與最近錯誤，可一鍵重新排入。
- **本月用量**：AI 呼叫次數、tokens、Email/電子報寄送量，與上月對比——異常燒量提早發現。
- **watchdog 告警**：每 10 分鐘自動巡檢（服務健康 + 排程逾時），異常時 Telegram 通知管理員，恢復時也會通知；相同問題不重複轟炸。

### 電子報寄送誠實化
- 寄送有任何失敗時，活動狀態現在會標示為「**部分失敗**」（紅色徽章，含失敗名單與原因），不再一律顯示「已寄出」；已部分寄出的活動同樣受刪除保護。

### 特權操作稽核
- 新增「稽核日誌」管理頁：MFA 重設、永久刪除、維護模式切換、MCP 權杖建立／撤銷等特權操作全數留痕（操作者、時間、對象、細節）。

## v7.3.0 — feat(security/ops): 名片影像私有化 + 資料保留清理 + 憑證加密（2026-07-04）

### 安全性
- **名片影像不再公開可讀**：先前任何人拿到連結即可讀取名片影像（含個資）。現改為私有儲存，站內顯示與 Telegram 傳圖一律改用短效簽名連結；資料庫既有資料完全不受影響。
- **Gmail 授權憑證改為加密儲存**（AES-256-GCM），並收回資料庫端非必要的存取授權——與 v7.2.8 的權杖保護同一標準。
- 電子報骨架的社群圖示改用自家儲存空間，不再依賴外部 CDN（外部服務失效不會再讓信件破圖）。

### 資料保留（新增每日清理排程）
- 垃圾桶中軟刪除逾 **90 天**的聯絡人自動永久刪除（含名片與相片檔案）。
- 過期的機器人會話、訊息去重紀錄、逾期 API 權杖、AI 產出快取自動清理。

### 其他
- **電子報「預覽文字」真正生效**：先前填了沒有作用，現在收件匣會正確顯示（測試寄送即可驗證）。
- 系統中介層遷移至 Next.js 16 新慣例（proxy），消除建置警告。

## v7.2.9 — fix(newsletter/reports/docs): 電子報編輯器修復 + 報表排程接線 + 全站文件更新（2026-07-03）

### 電子報編輯器（回應使用者回報）
- **修復「插入圖片變超大」**：直接編輯模式插入圖片後自動限制在版面寬度內（樣式會一併存入內容，寄出的信件同樣正確）；並新增**點選圖片即可調整寬度**（25% / 50% / 75% / 100%）。
- **新增格式工具列**：直接編輯模式現可調整**粗體、斜體、底線、字級（小/中/大）、文字顏色、對齊**，且不會破壞電子報版型。

### 報表排程
- **修復報表排程從未執行的問題**：先前「排程報表」建立後實際上不會寄出（背景無任何程序執行它）。現已接上每小時檢查機制，依各排程的週期設定自動產生並寄送；排程列表新增「上次執行」欄位顯示執行結果。

### 電子報數據保全
- SendGrid 事件（開信／點擊／退信等）現在會**完整落地保存**，包含先前被丟棄的**點擊連結網址**——為日後的成效分析頁鋪路，也避免歷史數據繼續流失。

### 文件全面更新（51 項勘誤與補齊）
- **新增 4 份功能文件（三語）**：相簿與人臉標記、AI 助理、會議前 Briefing、電子報活動與寄送。
- **修正會誤導操作的文件**：Telegram webhook 設定步驟（照舊文件操作會使 Bot 失效）、部署環境變數清單補齊（Portkey／SendGrid／CRON_SECRET 等）、首次登入指南補上必經的 MFA 設定步驟、報表頁移除不存在的「連結 Gmail」按鈕說明。
- **指令文件對齊實際程式**：補上 /lang、/stop、/ai、/li 等真實指令，修正 /add_back 等不存在的指令；Teams Bot 補上 /meet 會議排程說明。
- **修復說明書目錄壞連結**（4 處）、README 版本與功能清單更新至 v7.2.x、多份管理員文件對齊「可授權功能」權限模型。

## v7.2.8 — fix(security): 全系統安全稽核修復（權杖外洩／未授權端點／退訂合規等 21 項）（2026-07-03）

### 安全性（高風險，已修復）
- **修補 Microsoft 存取權杖外洩**：先前任何登入成員都能透過資料庫讀到所有人的 Microsoft（Outlook／行事曆）存取權杖與更新權杖。已在資料庫層收回 `users` 資料表的全欄位讀取權，改為僅開放必要欄位，權杖欄位不再對前端曝光。
- **後台 Hunter API 補上權限驗證**：`/api/admin/hunter`（Email 探勘）先前未驗證身分即可呼叫，現已要求 Super Admin 登入。
- **SendGrid 退訂匯入 API 補上權限驗證**：`/api/sendgrid/import-suppressions` 的手動匯入現要求 Super Admin 登入（排程仍走既有密鑰）。
- **修復電子報「退訂」完全無效的問題**：退訂與 email 取消訂閱端點先前被登入中介層攔截，導致收件人點退訂連結後系統從未真正記錄退訂（法遵風險）。現已將這些以簽章保護的公開端點正確放行。
- **Teams Bot Webhook 補上簽章驗證**：`/api/teams-bot` 先前只解碼未驗章的權杖，任何知道 App ID 的人都可偽造請求（標記任務完成、刪除會議草稿、建立行事曆）。現改為對 Bot Framework 公鑰做完整 RS256 簽章 + 發行者 + 對象 + 有效期驗證。
- **寄信身分改由登入狀態決定**：`/api/email/send` 先前信任前端傳入的使用者 ID，現一律由登入 session 推導寄件者。

### 資料一致性（已修復）
- **合併聯絡人不再靜默刪除退訂／黑名單資料**：合併時關於退訂、黑名單、AI 簡報、電子報訂閱等關聯資料先前可能被靜默刪除，導致已退訂或已退信的聯絡人再度變成可寄送。現改為正確地把這些資料重新指向保留的聯絡人。
- **電子報批次寄送更可靠**：加入「重複寄送防護」（已寄出的活動不會被再次整批寄出）、中斷後續寄的「已寄出略過」去重、以及寄送失敗時正確標記為 `failed`（先前可能卡在中間狀態）。
- **電子報草稿產生防重複／防過期**：草稿「彙整成活動」改為原子化領取快取，避免同一份預覽被重複建立為活動；並在送出前重新檢查，若素材在預覽後被刪除則擋下（不會再把已刪內容寄出）。
- **永久刪除聯絡人一併清除相片檔**：先前只清了名片圖，聯絡人相片檔會殘留在儲存空間，現已一併刪除。

### 韌性與錯誤處理（已修復）
- **名片 OCR 卡住列的重試判定修正**：先前以「建立時間」判斷卡住，會把積壓超過 10 分鐘的待處理名片誤判為卡住而重複處理；改以「領取時間」判定，並加上狀態守衛避免重複處理。手動「重新處理」也加上時間保護，不會誤中斷正在處理中的名片。
- **多個頁面補上錯誤處理**：Email 撰寫、任務、名片後台批次「認識日期」、掃描失敗清單等頁面，在儲存／送出失敗時會顯示明確錯誤並保留內容，不再靜默失敗或誤報成功；連線階段失效時自動導回登入頁。
- **掃描失敗清單修正**：清單依使用者正確過濾；刪除改為「先刪資料庫、成功後才清儲存檔」，避免資料還在卻先刪了圖。
- **個人 AI email 提示詞儲存位置修正**：先前可能存到錯誤的使用者鍵值，導致設定讀不回來，現已修正。
- 補上電子報名單「已退訂」狀態的三語系翻譯。
...
### 修復
- **電子報「AI 撰寫」不再靜默失敗**：前端補上錯誤處理,後端逾時或出錯時會顯示明確訊息,而不是按了沒反應。
- **AI 呼叫在 Portkey 失效時自動改走 `GEMINI_API_KEY` 直連**：名片 OCR、LinkedIn／任務／會議解析等核心功能,在 Portkey 中斷時仍可運作(不再全站單點故障)。
- **電子報 AI 產出會列出「被略過的素材」**：只有標題、沒有內文的素材無法被 AI 採用,預覽視窗現在會明白列出哪幾則被略過(補上內文再重新生成即可)。
- 多支重運算 AI route 補上 `maxDuration`,降低素材較多時的逾時風險。
- 電子報「下月預告」標籤在**產出端**也一併更正為「本月預告」(三語;上次僅改草稿頁 UI)。
- 清除殘留 `console.log`。

### 文件
- 新增電子報素材使用說明(站內 `/docs` 及 `docs/features/newsletter`,三語):草稿三區塊、拖拉排序、活動日期區間,以及「**素材需同時有標題與內文才會被 AI 採用**」的重點提醒。

## v7.2.5 — feat(newsletter): 電子報草稿拖拉排序、活動日期區間、回報檢查常駐化（2026-06-29）

### 電子報草稿頁（依 Luna Chang 回饋）
- **拖拉排序**：草稿卡片可拖曳調整先後順序、跨欄搬移，或直接拖進「📌 本期重點」設為當月焦點（本期重點僅一則，原本的會自動退回上月回顧）。支援觸控與鍵盤操作;放開即存(新增 `PATCH /api/newsletter/drafts/reorder`)。
- **活動日期區間**：事件可填「結束日期（選填）」,卡片與 AI 合成的電子報會顯示為區間（例如 BIO 6/22 – 6/25）。新增 `newsletter_drafts.event_date_end` 欄位。
- **欄位標籤更清楚**：「下月預告」改為「本月預告」（電子報預告的是當期月份,非下個月）,三語同步。

### 系統回報檢查
- **每日回報檢查改為常駐 Vercel cron**（`/api/cron/check-feedback`,每日 18:00）：取代會在 7 天後自動失效的 Claude Code session 排程,24/7 不中斷。有新回報會列出並附 AI 初步研判、寄給管理者;無新回報則寄「今日無新 issue」。
...
### 變更項目
- 第五十章 v7.2 對照表補上漏列的 **Task 224（chatbot 破壞性操作前端確認）** 至「未做」清單;並補註 Task 220 的 `pre_meeting` trigger 值已預留但無自動掃描 cron。
- 純文件變更。
...
### 變更項目
- **一張照片可標記多位聯絡人**（多對多）：相簿頁從「按聯絡人分組」改為「以照片為主體的網格」，每張照片顯示框到的人數與姓名
- Lightbox 內可**新增 / 移除**照片中的人（搜尋聯絡人即時標記），並預留 AI 建議的接受 / 拒絕介面
- 聯絡人詳情頁的照片改顯示「此聯絡人被框到的所有照片」（不再僅限上傳到該聯絡人的）
- 名片 Bot 與網頁上傳照片時，同步建立人臉標記
- i18n 三語（zh-TW / en / ja）新增相簿標記相關文字
- **資料庫**：新增 `photo_faces` 多對多表（已回填既有 56 筆照片）；並為後續 AI 人臉辨識預先建好 pgvector `face_embeddings` 表、比對函式與 `face_recognition_enabled` 全域開關（預設關閉）

### 備註
- AI 自動人臉偵測 / 辨識（Phase 2）資料層已就緒，推論部分（Supabase Edge Function）將於後續版本接上
