...
## Push to Telegram on Completion

As long as you had linked Telegram when the briefing was requested, the system pushes a Telegram message **once the briefing finishes**, containing the start of the summary and a link to the contact — so you can get the gist without returning to the web. This push applies to every request source:

- Clicking "Generate briefing" on the web
- Asking the Telegram AI assistant to queue a briefing
- Briefings auto-queued for upcoming meetings (see below)

If Telegram is not linked, or the request had no one to notify, nothing is pushed; the result still appears on the contact detail page as usual.
...
- If the user has linked Telegram, the system sends a Telegram summary of what was queued; once each briefing finishes, it also pushes a result summary (see "Push to Telegram on Completion" above).
- **Post-meeting outcome prompt**: after the meeting's start time has passed and its pre-meeting briefing is done, the system sends one more prompt (once per meeting) suggesting you log the outcome in one line with `/v`.
