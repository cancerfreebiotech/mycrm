...
mycrm 電子報模板骨架。從 listmonk 原版 HTML 重整出的乾淨版本，給 AI compose 流程（`/admin/newsletter/ai-compose` 管理頁面，已上線）當 base。
...
| `{{postal_address}}` | CAN-SPAM 實體地址（v7.9.4 起自 org settings 帶入，非寫死） | |
...
2. Logo / 社群 icons 搬到 Supabase Storage（擺脫 listmonk 和 mailerlite 的 CDN 依賴）— Logo 與社群 icons 均已搬移完成（v7.3.0）
3. ~~寫 AI compose endpoint：吃 stories + photos → 填這份骨架~~ ✅ 已完成
4. ~~寫 template import 腳本：把這 3 份 skeleton 寫進 `email_templates` 表（3 筆記錄：中/英/日）~~ ✅ 已完成
5. ~~設計 compose UI：管理員填入月份、stories、photos，點 AI 生成 → 預覽 → 存為 campaign~~ ✅ 已完成
